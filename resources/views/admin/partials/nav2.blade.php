<nav
    ...
    <div class="w-full mx-autp items-center flex justify-between md:flex-nowrap flex-wrap md:px-10 px-4">
        <div class="grid">
            <div class="refcode my-3">
                <a class="text-white text-sm uppercase hidden lg:inline-block font-semibold"
                    href=" {{ route('register', ['ref' => auth()->user()->ref_code ?? 000]) }}">
                    Ref code: {{ auth()->user()->ref_code ?? 'ref_code' }}
                </a>
            </div>
            <div class="reflink my-3">
                <a class="text-white text-sm uppercase hidden lg:inline-block font-semibold"
                    href=" {{ route('register', ['ref' => auth()->user()->ref_code ?? 000]) }}">
                    Ref Link: {{ route('register', ['ref' => auth()->user()->ref_code ?? 000]) }}
                </a>
            </div>
        </div>
                ...
                <div class="items-center flex">
                    <span
                        class="w-12 h-12 text-sm text-white bg-blueGray-200 inline-flex items-center justify-center rounded-full"><img
                            ...
                            src="/static/admin_assets/img/team-1-800x800.jpg" /></span>
                </div>
            </a>
