            ...
            <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded bg-white">
                    ...
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
                            <h3 class="font-semibold text-lg text-blueGray-700">
                                {{ ucfirst($reqUser->name) . "'s" }} Downlines 
                            </h3>
                        </div>
                    </div>
                </div>
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th
                                    ...
                                    {{-- actions column to be used later --}}
                                {{-- <th
                        ...
                        <tbody>
                            @foreach ($all_downlines as $reqUserDownline)
                                <tr>
                                    <th
                                    ...
                                    <td
                                        class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <i
                                            class="fas fa-circle {{ $reqUserDownline->is_admin ? 'text-orange-500 mr-2' : 'text-emerald-500 mr-2' }}"></i>
                                        <span class="uppercase"> {{ $reqUserDownline->is_admin ? 'admin' : 'client' }}</span>
                                    </td>
                                    <td
                                        class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <div class="flex">
                                            {{-- for the first downline --}}
                                            @if ($first_downline = $reqUserDownline->hasADownline())
                                                <img src="/static/admin_assets/img/team-1-800x800.jpg" alt="..."
                                                    title="{{ $first_downline->name }}"
                                                    class="w-10 h-10 rounded-full border-2 border-blueGray-50 shadow" />
                                            @endif
                                            {{-- for subsquent downlines --}}
                                            @if ($reqUserDownline->hasADownline() and $reqUserDownline->getDownlines()->count() > 1)
                                                @foreach ($reqUserDownline->getDownlines()->skip(1)->take(4) as $downline)
                                    ...
                                    <td
                                        class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $reqUserDownline->getDownlines()?->count() ?? 0 }}
                                    </td>

                                    <td
                                        class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        <a href="{{ route('register', ['ref' => $reqUserDownline->ref_code]) }}"> REFERRAL LINK</a>
                                    </td>
